...
for filename_gtf in os.listdir('.'):
    ...
    f_log = open('%s_ens%s_all.log'%(out_sp_code,out_version),'w')
    f_cdna = open('%s_ens%s_cdna_all.fa'%(out_sp_code,out_version),'w')
    f_ncdna = open('%s_ens%s_ncdna_all.fa'%(out_sp_code,out_version),'w')
    f_tx = open('%s_ens%s_tx_all.fa'%(out_sp_code,out_version),'w')
    f_prot = open('%s_ens%s_prot_all.fa'%(out_sp_code,out_version),'w')
    ...
    for tmp_pos in sorted(pos2gene.keys()):
        ...
        for tmp_tx_id in tmp_tx_seq.keys():
            gene_id = tmp_tx2gene[tmp_tx_id]['gene_id']
            gene_name = tmp_tx2gene[tmp_tx_id]['gene_name']

            if( len(tmp_prot_len) > 0 ):
                f_tx.write('>%s|%s gene:%s\n%s\n'%(gene_name,tmp_tx_id,gene_id,tmp_tx_seq[tmp_tx_id]))
                f_cdna.write('>%s|%s gene:%s\n%s\n'%(gene_name,tmp_tx_id,gene_id,tmp_tx_seq[tmp_tx_id]))
                for tmp_prot_id in sorted(tmp_prot_seq.keys()):
                    f_prot.write('>%s|%s gene:%s tx:%s\n%s\n'%(gene_name,tmp_prot_id,gene_id,tmp_tx_id,tmp_prot_seq[tmp_prot_id]))
                    count_cdna += 1
            else:
                f_tx.write('>%s|%s gene:%s\n%s\n'%(gene_name,tmp_tx_id,gene_id,tmp_tx_seq[tmp_tx_id]))
