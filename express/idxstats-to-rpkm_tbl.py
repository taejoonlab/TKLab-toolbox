...
def read_count(filename):
    ...
    for line in f:
        ...
        rv[t_id] = {'seqlen':tmp_seqlen, 'count': tmp_count, 'rpk':tmp_count*1000.0/tmp_seqlen}
    f.close()
    return rv
...
for tmp_group in group_list:
    for tmp_sample in exp[tmp_group].keys():
        tmp_lib_size = sum([x['count'] for x in exp[tmp_group][tmp_sample].values()])
        sample_lib_sizes[tmp_sample] = tmp_lib_size
        sys.stderr.write('Lib Size - %s: %.2f\n'%(tmp_sample, tmp_lib_size))

f_indiv_count = open('%s.indiv_rpkm.txt'%output_name,'w')
f_mean_count  = open('%s.mean_rpkm.txt'%output_name,'w')
f_low_count = open('%s.low_rpkm.txt'%output_name,'w')
    ...
    for tmp_group in group_list:
        ...
        for tmp_sample in exp[tmp_group].keys():
            if tmp_id in exp[tmp_group][tmp_sample]:
                tmp_count = exp[tmp_group][tmp_sample][tmp_id]['rpk']*1000000.0/sample_lib_sizes[tmp_sample]
