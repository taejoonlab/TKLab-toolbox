---
title: "RNA-seq analysis with Limma"
...
dirname_data <- "D:/Git/data/XenopusHybrid/orig.201806"
filename_count <- 'Kwon201408_XenopusHybrid_v201806.XT_maternal.txt'
min_cpm <- 1
min_cpm_count <- 2
...
Run Limma analysis to identify differentially expressed genes.

```{r}

y1 <- DGEList(counts=tbl_count, group=groups)
v1 <- voom(y1, design)
fit_lm <- lmFit(v1, design)
...
total_list <- topTable(fit_lm, coef=ncol(design), confint=TRUE, n=Inf)
DE_keep <- total_list$adj.P.Val <= 0.05 & abs(total_list$logFC) >= 1
DE_list <- total_list[DE_keep,]

filename_all <- paste(filename_count, 'limma_all.txt', sep='.')
write.table(total_list, filename_all, sep='\t')

filename_DE <- paste(filename_count, 'limma_DE.txt', sep='.')
write.table(DE_list, filename_DE, sep='\t')

logCPM <- cpm(tbl_count, log=TRUE)
filename_cpm <- paste(filename_count, 'limma_logCPM.txt', sep='.')
