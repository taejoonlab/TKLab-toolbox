...
for line in f_psl:
    ...
    if t_id not in q2t[q_id]:
        q2t[q_id][t_id] = dict()
        q2t_ratio[q_id][t_id] = 0
        q2t[q_id][t_id] = {'strand': strand,
                           'q_size': q_size, 't_size': t_size,
                           'block': ','.join(block_list),
                           'qstart': ','.join(qstart_list),
                           'tstart': ','.join(tstart_list),
...
h_out = '#Qid\tQlen\tT1_id\tT1_strand\tT1_ratio\t'
h_out += 'T1_block\tQ1_start\tT1_start\t'
h_out += 'T2_id\tT2_strand\tT2_ratio\tT2_block\t'
h_out += 'Q2_start\tT2_start'

f_out.write('%s\n' % h_out)
for tmp_q in q2t_ratio.keys():
    t_list = sorted(q2t_ratio[tmp_q].keys(), key=q2t_ratio[tmp_q].get,
                    reverse=True)
    t1 = t_list[0]
    ...
    f_out.write("%s\t%d\t%s\t%s\t%.3f\t%s\t%s\t%s\t" %
                (tmp_q, q_size, t1, strand1, ratio1, block1, qstart1, tstart1))
