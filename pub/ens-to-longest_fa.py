...
usage_mesg = 'Usage : ens-to-longest.py <Species prefix (e.g. Homo)>\n'
...
for filename_gtf in os.listdir('.'):
    ...
    f_cdna = open('%s_cdna_longest.fa' % ens_name_base, 'w')
    f_ncdna = open('%s_ncdna_longest.fa' % ens_name_base, 'w')
    f_tx = open('%s_tx_longest.fa' % ens_name_base, 'w')
    f_prot = open('%s_prot_longest.fa' % ens_name_base, 'w')
    ...
    for tmp_pos in sorted(pos2gene.keys()):
        ...
        if len(tmp_prot_len) > 0:
            ...
            for tmp_prot_id in prot_sorted:
                if tmp_prot_len[tmp_prot_id] < longest_prot_len:
                    continue
            ...
            f_tx.write('>%s\n%s\n' % (tmp_tx_h, tmp_tx_seq[longest_tx_id]))
            f_cdna.write('>%s\n%s\n' % (tmp_tx_h, tmp_tx_seq[longest_tx_id]))
